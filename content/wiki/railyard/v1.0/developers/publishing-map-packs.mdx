...
If you are a map creator and have made multiple maps or are planning to, you can host them all on the same repository via a custom `update.json` file. In this file, you can place all the necessary information required for Railyard to fetch updates for your map.
...
For mods, each version entry must also include a `manifest` field pointing to the mod's `manifest.json` for that release. Railyard uses this to validate dependencies without downloading the full ZIP.
...
Custom URL:   Required ONLY if you selected Custom URL. The full URL to your self-hosted update.json file. The file
              ...
              found at: https://subwaybuildermodded.com/wiki/railyard/v1.0/developers/publishing-map-packs#custom-update-schema-template.
