...
const { Button, Card, CardHeader, CardTitle, CardContent } = api.utils
...
```tsx
const { Button } = api.utils.components as Record<
  string,
...
```tsx
const { Card, CardHeader, CardTitle, CardContent } = api.utils
  .components as Record<string, React.ComponentType<any>>;
