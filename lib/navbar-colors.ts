...
export function getNavbarConfiguredColors(
  config?: NavbarColorConfig,
): NavbarItemColors | undefined {
  if (!config) return undefined;
  return config.colors ?? config.color;
}

export function getNavbarThemeColors(
  config: NavbarColorConfig | undefined,
  isDark: boolean,
) {
