...
const WIKI_CARD_SELECTOR = "[data-wiki-card]"
const WIKI_CARD_HEADING_SELECTOR = "[data-wiki-card-heading]"

function syncWikiCardRowHeadingHeights(container: HTMLElement) {
  const cards = Array.from(container.querySelectorAll<HTMLElement>(WIKI_CARD_SELECTOR))

  if (!cards.length) {
    return
  }

  cards.forEach((card) => card.style.removeProperty("--wiki-card-heading-height"))

  const containerRect = container.getBoundingClientRect()
  const rows = new Map<number, Array<{ card: HTMLElement; heading: HTMLElement }>>()

  cards.forEach((card) => {
    const heading = card.querySelector<HTMLElement>(WIKI_CARD_HEADING_SELECTOR)
    if (!heading) {
      return
    }
  ...
  rows.forEach((rowCards) => {
    ...
    rowCards.forEach(({ card }) => {
      card.style.setProperty("--wiki-card-heading-height", headingHeightValue)
    })
  })
}
...
export function WikiCardGrid({
  ...
  React.useLayoutEffect(() => {
    ...
    const measure = () => {
      syncWikiCardRowHeadingHeights(grid)
    }
    ...
    grid.querySelectorAll<HTMLElement>(WIKI_CARD_SELECTOR).forEach((card) => {
      resizeObserver.observe(card)
    })

    grid.querySelectorAll<HTMLElement>(WIKI_CARD_HEADING_SELECTOR).forEach((heading) => {
    ...
    <Link href={href} className="block h-full outline-none">
      ...
      >
        <CardHeader className="gap-2 px-5 py-2">
          <div
            data-wiki-card-heading
            className="flex min-h-[var(--wiki-card-heading-height,auto)] items-center"
          >
