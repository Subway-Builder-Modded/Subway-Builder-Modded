...
import { PROJECT_COLOR_SCHEMES, SHARED_TEXT_COLOR, SHARED_TEXT_COLOR_INVERTED } from "@/config/theme/colors"
import { hexAlpha } from "@/lib/color"
import { cn } from "@/lib/utils"

type ThemedShowcaseCardVariant = "home" | "hub" | "docs" | "credits" | "version" | "header"
...
function resolvePaletteFromScheme(scheme?: Exclude<SiteColorSchemeId, "default">): ThemedShowcaseCardPalette {
  if (!scheme) {
    return {
      accent: {
...
function buildCardVars(
  ...
  const ringLightFallback = "color-mix(in srgb, var(--ts-card-accent-light) 38%, transparent)"
  const ringDarkFallback = "color-mix(in srgb, var(--ts-card-accent-dark) 45%, transparent)"
  const softLightFallback = "color-mix(in srgb, var(--ts-card-accent-light) 20%, transparent)"
  const softDarkFallback = "color-mix(in srgb, var(--ts-card-accent-dark) 24%, transparent)"
  ...
  return {
    ...
    ["--ts-card-ring-light" as string]: deriveAlpha(accent.light, 0.38, ringLightFallback),
    ["--ts-card-ring-dark" as string]: deriveAlpha(accent.dark, 0.45, ringDarkFallback),
...
export function ThemedShowcaseCard({
  ...
  return (
    <article
      className={cn("group relative overflow-hidden", variantClasses.card, className)}
      style={{
    ...
    >
      <span
        className={cn(
          ...
          variantClasses.interactive ? "opacity-0 transition-opacity duration-300 group-hover:opacity-100" : "opacity-0",
          variantClasses.ring,
        )}
      />
      <span
      ...
      {variantClasses.spotB !== "hidden" ? (
        <span
          className={cn(
            "pointer-events-none absolute bg-[var(--ts-card-soft-light)] dark:bg-[var(--ts-card-soft-dark)]",
            variantClasses.interactive ? "opacity-0 transition-opacity duration-300 group-hover:opacity-80" : "opacity-0",
