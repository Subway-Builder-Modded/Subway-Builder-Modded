...
import {
  getNavbarConfiguredColors,
  getNavbarThemeColors,
} from '@/lib/navbar-colors';
...
export function ThemeMenu({
  ...
  const [hoveredThemeValue, setHoveredThemeValue] =
    React.useState<ThemeValue | null>(null);
  ...
  const themeOptions = React.useMemo(() => {
    return themes.map((entry) => {
      const configured = items?.find(
        (item) => item.id === `theme-${entry.value}`,
      );
      const configuredColors = getNavbarConfiguredColors(configured);
      const configuredIcon = configured?.icon;
      const Icon =
        configuredIcon &&
        !(typeof configuredIcon === 'object' && 'type' in configuredIcon)
  ...
  const lockMenuColors = React.useCallback(() => {
    const menuSurface = document.querySelector<HTMLElement>(
  ...
  return (
    <DropdownMenu open={menuOpen} onOpenChange={handleOpenChange} modal={false}>
      ...
      >
        {themeOptions.map(
          ({ value, label, Icon, hoverColors, configuredColors }) => {
